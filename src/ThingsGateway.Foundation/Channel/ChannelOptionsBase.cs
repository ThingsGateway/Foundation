    ...
    public abstract class ChannelOptionsBase : IValidatableObject
    {
        ...
        public virtual IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
        {
            if (ChannelType == ChannelTypeEnum.TcpClient)
            {
                ...
                {
                    yield return new ValidationResult("Remote IP Address cannot be empty", new[] { nameof(RemoteUrl) });
                }
            }
            else if (ChannelType == ChannelTypeEnum.TcpService)
            {
                ...
                {
                    yield return new ValidationResult("Local Bind IP Address cannot be empty", new[] { nameof(BindUrl) });
                }
            }
            else if (ChannelType == ChannelTypeEnum.UdpSession)
            {
                ...
                {
                    yield return new ValidationResult("Remote IP Address or Local Bind IP Address cannot be empty", new[] { nameof(BindUrl), nameof(RemoteUrl) });
                }
            }
            else if (ChannelType == ChannelTypeEnum.SerialPort)
            {
                ...
                {
                    yield return new ValidationResult("Serial Port cannot be empty", new[] { nameof(PortName) });
                }
            }
        }
        ...
        public override string? ToString()
        {
            switch (ChannelType)
            {
                ...
                case ChannelTypeEnum.UdpSession:
                    return string.IsNullOrEmpty(BindUrl) ? RemoteUrl : BindUrl;
