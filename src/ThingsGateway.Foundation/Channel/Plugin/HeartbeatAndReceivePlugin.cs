...
public sealed class HeartbeatAndReceivePlugin : PluginBase, ITcpConnectedPlugin, ITcpReceivingPlugin, ITcpClosedPlugin
{
    public string? DtuId
    {
        get
    ...
    public async Task OnTcpConnected(ITcpSession client, ConnectedEventArgs e)
    {
        ...
        if (string.IsNullOrWhiteSpace(DtuId)) return;
                ...
                _task = Task.Run(async () =>
                 {
                     ...
                     while (SendHeartbeat)
                     {
                         ...
                         try
                         {
                             if (DateTime.UtcNow - tcpClient.LastSentTime < TimeSpan.FromMilliseconds(200))
                             {
    ...
    public Task OnTcpReceiving(ITcpSession client, BytesReaderEventArgs e)
    {
        ...
        if (string.IsNullOrWhiteSpace(DtuId)) return Task.CompletedTask;
