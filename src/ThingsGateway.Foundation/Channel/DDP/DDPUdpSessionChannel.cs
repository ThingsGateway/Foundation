...
public class DDPUdpSessionChannel : UdpSessionChannel, IClientChannel, IDtuUdpSessionChannel
{
    ...
    NonBlockingDictionary<EndPoint, string> IdDict { get; set; } = new();
    NonBlockingDictionary<string, EndPoint> EndPointDcit { get; set; } = new();
    public bool TryGetId(EndPoint endPoint, [MaybeNullWhen(false)] out string id)
    {
