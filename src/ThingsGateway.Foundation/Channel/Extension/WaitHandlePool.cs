    ...
    where T : class, IWaitHandle
{
    ...
    private readonly Dictionary<int, AsyncWaitData<T>> m_waitDic = new();
    ...
    private AsyncWaitData<T> GetOrCreate(int sign, T pending)
    {
        lock (_lock)
        {
            if (this.s_pool.Count > 0)
            {
                var item = this.s_pool.Pop();
    ...
    public AsyncWaitData<T> GetWaitDataAsync(T result, bool autoSign = true)
    {
        ...
        lock (_lock)
        {
            ...
            {
                throw new InvalidOperationException($"The sign '{result.Sign}' is already in use.");
            }
        }
        return waitData;
    ...
    public AsyncWaitData<T> GetWaitDataAsync(out int sign)
    {
        ...
        lock (_lock)
        {
            if (!this.m_waitDic.TryAdd(sign, waitData))
            {
                throw new InvalidOperationException($"The sign '{sign}' is already in use.");
            }
        }
        return waitData;
    }
