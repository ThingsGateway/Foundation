...
public static class TextFileReader
{
    ...
    public static OperResult<string[]> GetLogFiles(string directoryPath)
    {
        OperResult<string[]> result = new(); // 初始化结果对象
        ...
        lock (fileLock)
        {
            try
            {
                ...
                if (_cache.TryGetValue(cacheKey, out var cachedData))
                {
                    ...
                    {
                        return new OperResult<LogData[]>() { Content = cachedData.LogDatas };
                    }
                    else
                    {
                ...
                return new OperResult<LogData[]>() { Content = result };
            }
            catch (Exception ex)
            {
                return new OperResult<LogData[]>(ex);
            }
        }
    }
    ...
    {
        using ValueListBuilder<string> items = new();
