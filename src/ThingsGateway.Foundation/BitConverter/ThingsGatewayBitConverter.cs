...
[Mapper(EnumMappingStrategy = EnumMappingStrategy.ByName, RequiredMappingStrategy = RequiredMappingStrategy.None)]
internal static partial class Mapper
{
    public static partial ThingsGatewayBitConverter.ThingsGatewayBitConverterDefault Adapt(this IThingsGatewayBitConverter src);
}
...
/// 或将一个字节数组转换为指定端基数据类型。
/// </summary>
public abstract class ThingsGatewayBitConverter : IThingsGatewayBitConverter
{
    /// <inheritdoc/>

    [System.Text.Json.Serialization.JsonConverter(typeof(SystemTextJsonEncodingConverter))]
    [Newtonsoft.Json.JsonConverter(typeof(NewtonsoftJsonEncodingConverter))]
    ...
    /// 以大端
    /// </summary>
    public static readonly ThingsGatewayBitConverter BigEndian = new ThingsGatewayBitConverterDefault(EndianType.Big);

    /// <summary>
    /// 以小端
    /// </summary>
    public static readonly ThingsGatewayBitConverter LittleEndian = new ThingsGatewayBitConverterDefault(EndianType.Little);


    internal sealed class ThingsGatewayBitConverterDefault : ThingsGatewayBitConverter
    {
        public ThingsGatewayBitConverterDefault()
        {
        }
        public ThingsGatewayBitConverterDefault(EndianType endianType) : base(endianType)
        {
        }
    ...
    {
        if (string.IsNullOrEmpty(registerAddress)) return this;
        ...
        bool? wstring;
        int? stringlength;
        BcdFormatEnum? bcdFormat;

        // 更新设备地址为去除附加信息后的地址
        registerAddress = GetConverterConfig(strs, registerAddress.Length, out dataFormat, out encoding, out wstring, out stringlength, out bcdFormat);
        // 如果没有解析出任何附加信息，则直接返回默认的数据转换器
        if (bcdFormat == null && stringlength == null && encoding == null && dataFormat == null && wstring == null)
        {
            return this;
        }
        IThingsGatewayBitConverter? converter = null;
        if (this.GetType() == typeof(ThingsGatewayBitConverter))
        {
            converter = new ThingsGatewayBitConverterDefault();
        }
        else
        {
    ...
    public static string GetConverterConfig(string[] strs, int maxLength, out DataFormatEnum? dataFormat, out Encoding? encoding, out bool? wstring, out int? stringlength, out BcdFormatEnum? bcdFormat)
    {
        using var sb = new ValueStringBuilder(maxLength);
        dataFormat = null;
        encoding = null;
        wstring = null;
        stringlength = null;
        ...
        foreach (var str in strs)
        {
            ...
            else if (str.StartsWith("len=", StringComparison.OrdinalIgnoreCase))
            {
                ...
                stringlength = string.IsNullOrEmpty(lenStr) ? null : Convert.ToUInt16(lenStr);
            }
            // 解析 bcdFormat
    ...
    protected abstract IThingsGatewayBitConverter CreateInstance();
    ...
    public virtual Memory<byte> GetBytes(string value)
    {
        ...
        if (StringLength != null)
        {
            if (BcdFormat != null)
            {
                var bytes = value.GetBytesByBCD(BcdFormat.Value).AsMemory();
    ...
    public virtual string ToString(ReadOnlySpan<byte> buffer, int offset, int length)
    {
        ...
        if (BcdFormat != null)
        {
            return IsStringReverseByteWord ? buffer.BytesReverseByWord().GetBcdValue(BcdFormat.Value) : buffer.GetBcdValue(BcdFormat.Value);
        }
        else
        {
            return IsStringReverseByteWord ?
buffer.BytesReverseByWord().ToString(EncodingValue).AsSpan().TrimEnd().TrimInvisible() :
buffer.ToString(EncodingValue).AsSpan().TrimEnd().TrimInvisible();
        }
    }
    ...
    public virtual bool[] ToBoolean(ReadOnlySpan<byte> buffer, int offset, int len, bool isReverse = false)
    {
        ...
        if (isReverse)
            buffer = buffer.BytesReverseByWord();
        for (int i = 0; i < result.Length; i++)
        {
            result[i] = ToBoolean(buffer, offset + i, false);
        }
        return result;
    }
    ...
    private byte[] GetTBytes<T>(T value) where T : unmanaged
    {
        ...
        {
            throw new NotSupportedException("value cannot be bool");
        }
        var size = Unsafe.SizeOf<T>();
