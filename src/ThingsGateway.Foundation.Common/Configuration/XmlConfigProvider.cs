...
public class XmlConfigProvider : FileConfigProvider
{
    ...
    protected override void OnRead(String fileName, IConfigSection section)
    {
        ...
        XmlConfigProvider.ReadNode(reader, section);
    ...
    private static void ReadNode(XmlReader reader, IConfigSection section)
    {
        while (true)
        {
            ...
            if (reader.NodeType is XmlNodeType.Element or XmlNodeType.Comment)
                XmlConfigProvider.ReadNode(reader, cfg);
    ...
    public override String GetString(IConfigSection? section = null)
    {
        ...
        XmlConfigProvider.WriteNode(writer, RootName, section);
        ...
        foreach (var item in section.Childs.ToArray())
        {
            ...
            if (cs != null)
            {
                ...
                if (cs.Count >= 2 && cs[0].Key == cs[1].Key)
                {
                    ...
                    {
                        if (!elm.Key.IsNullOrEmpty()) XmlConfigProvider.WriteAttributeNode(writer, elm.Key, elm);
                    }
                    writer.WriteEndElement();
                }
                else
                {
                    XmlConfigProvider.WriteNode(writer, item.Key, item);
                }
            }
            else
            {
