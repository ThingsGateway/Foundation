...
public class ExpiringDictionary<TKey, TValue> : IDisposable where TKey : notnull
{
    /// <summary>缓存项</summary>
    ...
    private bool tryDispose = false;
    IEqualityComparer<TKey>? comparer;
    public ExpiringDictionary(int expire = 60, IEqualityComparer<TKey>? comparer = null, bool tryDispose = false)
    {
    ...
    public TValue GetOrAdd(TKey key, Func<TValue?> func)
    {
        ...
        do
        {
            ...
            item ??= new CacheItem(func(), defaultExpire);
        }
        while (!_dict.TryAdd(key, item));
    ...
    {
        var resut = _dict.TryRemove(key, out var data);
        data?.Value.TryDispose();
        return resut;
    }
    ...
    private void TimerClear(object? state)
    {
        ...
        // 这里先计算，性能很重要
        using var toDels = new ValueListBuilder<TKey>();
        ...
        // 确认删除
        foreach (var item in toDels.AsSpan())
        {
            _dict.TryRemove(item, out var data);
            if (tryDispose)
                data?.Value.TryDispose();
        }
    }
