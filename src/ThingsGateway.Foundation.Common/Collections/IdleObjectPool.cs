...
namespace ThingsGateway.Foundation.Common.Collections;

/// <summary>资源池。支持空闲释放，主要用于数据库连接池和网络连接池</summary>
public class IdleObjectPool<T> : DisposeBase, IPool<T> where T : notnull
{
    #region 属性
    /// <summary>名称</summary>
    ...
    public IdleObjectPool()
    {
        var str = GetType().Name;
        if (str.Contains('`')) str = str.Substring(null, "`");
        if (str != "IdleObjectPool")
            Name = str;
        else
            Name = $"IdleObjectPool<{typeof(T).Name}>";
    ...
    public virtual Boolean Return(T value)
    {
        ...
        if (value is DisposeBase db && db.DisposedValue)
        {
            return false;
        }
