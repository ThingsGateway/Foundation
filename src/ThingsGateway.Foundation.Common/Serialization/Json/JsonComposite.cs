...
using ThingsGateway.Foundation.Common.Serialization.Interface;
...
public class JsonComposite : JsonHandlerBase
{
    ...
    public override Boolean Write(Object? value, Type type)
    {
        ...
        {
            if (IgnoreMembers?.Contains(member.Name) == true) continue;

            var mtype = member.GetMemberType();
            context.Member = Host.Member = member;

            var v = value.GetValueEx(member);
    ...
    public override Boolean TryRead(Type type, ref Object? value)
    {
        ...
        if (!type.IsFrom<Object>()) return false;
    ...
    {
        if (Host.UseProperty)
            return type.GetPropertiesEx(baseFirst).Cast<MemberInfo>().ToList();
        else
            return type.GetFieldsEx(baseFirst).Cast<MemberInfo>().ToList();
    }
