...
public static class JsonHelper
{
    ...
    public static T? Convert<T>(this IJsonHost jsonHost, Object obj)
    {
        ...
        if (obj.GetType().IsFrom<T>()) return (T)obj;
...
public class SystemJson : IJsonHost
{
    ...
    public static JsonSerializerOptions GetDefaultOptions()
    {
        ...
        opt.Converters.Add(new SystemTextJsonLocalTimeConverter());
        opt.Converters.Add(new SystemTextJsonTypeConverter());
    ...
    static SystemJson()
    {
        ...
        opt.TypeInfoResolver = new System.Text.Json.Serialization.Metadata.DefaultJsonTypeInfoResolver
        {
            Modifiers = {
                ...
                new ServiceTypeResolver{ GetServiceProvider = () => App.Provider }.Modifier,
            }
        };
#endif
        Options = opt;
    ...
    {
        var key = $"{indented}_{nullValue}_{camelCase}";
        return _optionsCache.GetOrAdd(key, _ =>
        {
            var options = new JsonSerializerOptions(jsonSerializerOptions)
            {
                WriteIndented = indented,
                DefaultIgnoreCondition = nullValue ? JsonIgnoreCondition.Never : JsonIgnoreCondition.WhenWritingDefault,
    ...
    /// <param name="camelCase">是否驼峰命名。默认false</param>
    /// <returns></returns>
    public string Write(object value, bool indented = false, bool nullValue = true, bool camelCase = false)
    {
    ...
    {
        var options = GetOptions(Options, jsonOptions.WriteIndented, jsonOptions.IgnoreNullValues, jsonOptions.CamelCase);
        ...
        {
            options.Converters.Add(new SystemTextJsonSafeInt64Converter());
            options.Converters.Add(new SystemTextJsonSafeUInt64Converter());
        }
#endif
