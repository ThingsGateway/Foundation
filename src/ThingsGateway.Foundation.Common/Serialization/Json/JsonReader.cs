...
public class JsonReader
{
    ...
    public IServiceProvider Provider { get; set; } = App.Provider;
    #endregion
    ...
    public Object? ToObject(Object? jobj, Type? type, Object? target)
    {
        ...
        if (jobj == null || type?.IsAssignableFrom(jobj.GetType()) != false) return jobj;
    ...
    {
        if (type == typeof(NameValueCollection)) return JsonReader.CreateNV(dic);
        if (type == typeof(StringDictionary)) return JsonReader.CreateSD(dic);
        if (type == typeof(Object)) return dic;

        target ??= Provider.GetService(type) ?? Provider.CreateInstance(type) ?? type.CreateInstance();
        if (target == null) return null;

        if (type.IsDictionary()) return JsonReader.CreateDic(dic, type, target);
        ...
        // 遍历所有可用于序列化的属性
        var props = target.GetType().GetPropertiesEx(true).ToDictionary(e => SerialHelper.GetName(e), e => e);
        foreach (var item in dic)
        {
            ...
            if (pt.IsBaseType())
                target.SetValueEx(pi, ChangeType(v, pt));
            else
            {
                var orig = target.GetValueEx(pi);
                var val = ToObject(v, pt, orig);
                if (val != orig)
                {

                    target.SetValueEx(pi, val);
                }
            }
        }
        return target;
    ...
    private Object? ChangeType(Object value, Type type)
    {
        ...
        if (value is String str2) return str2.ChangeTypeEx(type);
        if (type.IsBaseType()) return value.ChangeTypeEx(type);

        return null;
    }

    private static StringDictionary CreateSD(IDictionary<String, Object> dic)
    {
    ...
    private static NameValueCollection CreateNV(IDictionary<String, Object> dic)
    {
    ...
    private static IDictionary? CreateDic(IDictionary<String, Object> dic, Type type, Object obj)
    {
        ...
        if (type.IsGenericType && type.GetGenericArguments().Length >= 2)
        {
            ...
            foreach (var item in dic)
            {
                nv.Add(item.Key, tval == null ? item.Value : item.Value.ChangeTypeEx(tval));
            }
        }
        else
        {
    ...
    private static Int32 CreateInteger(String str, Int32 index, Int32 count)
    {
        var num = 0;
        var neg = false;
    ...
    private DateTime CreateDateTime(Object value)
    {
        ...
        if (value is String str)
        {
            ...
            year = JsonReader.CreateInteger(str, 0, 4);
            month = JsonReader.CreateInteger(str, 5, 2);
            day = JsonReader.CreateInteger(str, 8, 2);
            if (str.Length >= 19)
            {
                hour = JsonReader.CreateInteger(str, 11, 2);
                min = JsonReader.CreateInteger(str, 14, 2);
                sec = JsonReader.CreateInteger(str, 17, 2);
                if (str.Length > 21 && str[19] == '.')
                    ms = JsonReader.CreateInteger(str, 20, 3);
