...
public class JsonParser
{
                ...
                case 'u':
                    {
                        ...
                        // 分析32位十六进制数字
                        var codePoint = JsonParser.ParseUnicode(_json[index], _json[index + 1], _json[index + 2], _json[index + 3]);
    ...
    private static UInt32 ParseSingleChar(Char c1, UInt32 multipliyer)
    {
        UInt32 p1 = 0;
        if (c1 >= '0' && c1 <= '9')
    ...
    private static UInt32 ParseUnicode(Char c1, Char c2, Char c3, Char c4)
    {
        var p1 = JsonParser.ParseSingleChar(c1, 0x1000);
        var p2 = JsonParser.ParseSingleChar(c2, 0x100);
        var p3 = JsonParser.ParseSingleChar(c3, 0x10);
        var p4 = JsonParser.ParseSingleChar(c4, 1);

        return p1 + p2 + p3 + p4;
    }

    private static Int64 CreateLong(String s)
    {
        Int64 num = 0;
        var neg = false;
        foreach (var cc in s)
