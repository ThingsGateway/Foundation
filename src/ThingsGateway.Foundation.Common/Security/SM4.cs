...
namespace ThingsGateway.Foundation.Common.Security;
...
public class SM4Transform : ICryptoTransform
{
    ...
    private void ExpandKey(Boolean forEncryption, Byte[] key)
    {
        ...
        if (forEncryption)
        {
            rk[0] = K0 ^ SM4Transform.T_ap(K1 ^ K2 ^ K3 ^ CK[0]);
            rk[1] = K1 ^ SM4Transform.T_ap(K2 ^ K3 ^ rk[0] ^ CK[1]);
            rk[2] = K2 ^ SM4Transform.T_ap(K3 ^ rk[0] ^ rk[1] ^ CK[2]);
            rk[3] = K3 ^ SM4Transform.T_ap(rk[0] ^ rk[1] ^ rk[2] ^ CK[3]);
            for (var i = 4; i < 32; ++i)
            {
                rk[i] = rk[i - 4] ^ SM4Transform.T_ap(rk[i - 3] ^ rk[i - 2] ^ rk[i - 1] ^ CK[i]);
            }
        }
        else
        {
            rk[31] = K0 ^ SM4Transform.T_ap(K1 ^ K2 ^ K3 ^ CK[0]);
            rk[30] = K1 ^ SM4Transform.T_ap(K2 ^ K3 ^ rk[31] ^ CK[1]);
            rk[29] = K2 ^ SM4Transform.T_ap(K3 ^ rk[31] ^ rk[30] ^ CK[2]);
            rk[28] = K3 ^ SM4Transform.T_ap(rk[31] ^ rk[30] ^ rk[29] ^ CK[3]);
            for (var i = 27; i >= 0; --i)
            {
                rk[i] = rk[i + 4] ^ SM4Transform.T_ap(rk[i + 3] ^ rk[i + 2] ^ rk[i + 1] ^ CK[31 - i]);
            }
        }
    }
