...
public partial class MachineInfo
{
    ...
    public void Init()
    {
        ...
        if (string.IsNullOrEmpty(OSVersion)) OSVersion = osv.Version.ToString();
        if (string.IsNullOrEmpty(OSName)) OSName = (osv.ToString()).TrimStart("Microsoft").TrimEnd(OSVersion).Trim().ToString();
        Guid ??= string.Empty;
        ...
        // 裁剪不可见字符，顺带去掉两头空白
        OSName = OSName?.Trim().TrimInvisible();
        OSVersion = OSVersion?.Trim().TrimInvisible();
        Product = Product?.Trim().TrimInvisible();
        Processor = Processor?.Trim().TrimInvisible();
        UUID = UUID?.Trim().TrimInvisible();
        Guid = Guid?.Trim().TrimInvisible();
        Serial = Serial?.Trim().TrimInvisible();
        Board = Board?.Trim().TrimInvisible();
        DiskID = DiskID?.Trim().TrimInvisible();

        // 无法读取系统标识时，随机生成一个guid，借助文件缓存确保其不变
        if (string.IsNullOrEmpty(Guid)) Guid = $"0-{System.Guid.NewGuid().ToString()}";
        if (string.IsNullOrEmpty(UUID)) UUID = $"0-{System.Guid.NewGuid().ToString()}";
    ...
    private void LoadWindowsInfo()
    {
        ...
        if (reg != null)
        {
            ...
            if (!string.IsNullOrEmpty(str) && !str.EqualIgnoreCase("FFFFFFFF-FFFF-FFFF-FFFF-FFFFFFFFFFFF")) UUID = str;
        }
        ...
        if (reg != null)
        {
            Product = (reg.GetValue("SystemProductName")?.ToString())?.Replace("System Product Name", null);
            if (string.IsNullOrEmpty(Product)) Product = reg.GetValue("BaseBoardProduct") + string.Empty;

            Vendor = reg.GetValue("SystemManufacturer") + string.Empty;
            if (string.IsNullOrEmpty(Vendor)) Vendor = reg.GetValue("ASUSTeK COMPUTER INC.") + string.Empty;
        }
        ...
        if (string.IsNullOrEmpty(UUID) || UUID == Guid || string.IsNullOrEmpty(Vendor))
        {
            ...
            if (csproduct != null)
            {
                if (csproduct.TryGetValue("Name", out str) && !string.IsNullOrEmpty(str) && string.IsNullOrEmpty(Product)) Product = str;
                if (csproduct.TryGetValue("UUID", out str) && !string.IsNullOrEmpty(str)) UUID = str;
                if (csproduct.TryGetValue("Vendor", out str) && !string.IsNullOrEmpty(str)) Vendor = str;
            }
        }
#else
        str = ProcessHelper.Execute("reg", @"query HKLM\SOFTWARE\Microsoft\Cryptography /v MachineGuid", 0, false);
        if (!string.IsNullOrEmpty(str) && str!.Contains("REG_SZ")) Guid = str.Substring("REG_SZ", null).Trim();
        ...
        try
        {
            ...
            // 系统名取WMI可能出错
            OSName = ci.OSFullName?.Replace("®", null).TrimStart("Microsoft").Trim().ToString();
        ...
        if (os is { Count: > 0 })
        {
            if (os.TryGetValue("Caption", out str)) OSName = str.TrimStart("Microsoft").Trim().ToString();
        ...
        if (!string.IsNullOrEmpty(sn) && !sn.EqualIgnoreCase("System Serial Number")) Serial = sn;
        ...
        if (string.IsNullOrEmpty(OSName))
            OSName = RuntimeInformation.OSDescription.TrimStart("Microsoft").Trim().ToString();
    ...
    private void LoadLinuxInfo()
    {
        ...
        if (dic != null)
        {
            ...
            {
                Processor = str?.TrimStart("vendor ").ToString();
            }
            ...
            //if (device.TryGetValue("Fingerprint", out str) && !string.IsNullOrEmpty(str))
            //    CpuID = str;
            if (dic.TryGetValue("Serial", out str) && !string.IsNullOrEmpty(str) && !string.IsNullOrEmpty(str!.Trim('0')))
                UUID = str;
        }
        ...
        if (TryRead(mid, out var value))
            Guid = value;
        else if (device.TryGetValue("android_id", out str) && !string.IsNullOrEmpty(str) && str != "unknown")
            Guid = str;
        ...
        if (string.IsNullOrEmpty(prd) && TryRead("/sys/class/dmi/id/product_name", out var product_name))
        {
            ...
            if (TryRead("/sys/class/dmi/id/sys_vendor", out var vendor) && !string.IsNullOrEmpty(vendor))
            {
                Vendor = vendor;

                if (!string.IsNullOrEmpty(product_name) && !product_name.Contains(vendor))
                {
                    // 红帽KVM太流行，细化处理
                    if (product_name == "KVM" && vendor == "Red Hat" &&
                        TryRead("/sys/class/dmi/id/product_version", out var ver) && !string.IsNullOrEmpty(ver))
                    {
                        var p = ver.IndexOf('(');
        ...
        if (TryRead(file, out value) && !string.IsNullOrEmpty(value))
        {
            Board = value;
        }
        else
        ...
        if (disks.Count > 0) DiskID = disks.Where(e => !string.IsNullOrEmpty(e)).Join(",");
    ...
    private void RefreshWindows()
    {
        ...
        if (!_excludes.Contains(nameof(Temperature)))
        {
            ...
            if (temp?.Count > 0)
            {
                if (temp.TryGetValue("CurrentTemperature", out var str) && !string.IsNullOrEmpty(str))
                    Temperature = (str.SplitAsInt().Average() - 2732) / 10.0;
            }
            else
            {
        ...
        else if (!_excludes.Contains(nameof(Battery)))
        {
            ...
            if (battery?.Count > 0)
            {
                if (battery.TryGetValue("EstimatedChargeRemaining", out var str) && !string.IsNullOrEmpty(str))
                    Battery = str.SplitAsInt().Average() / 100.0;
            }
            else
            {
    ...
    private void RefreshLinux()
    {
        ...
        else
        {
            ...
            if (dic != null)
            {
                if (dic.TryGetValue("MemTotal", out var str) && !string.IsNullOrEmpty(str))
                    Memory = (UInt64)(str.TrimEnd(" kB").ToLong() / 1024.0);

                ulong ma = 0;
                if (dic.TryGetValue("MemAvailable", out str) && !string.IsNullOrEmpty(str))
                {
        ...
        {
            if (!String.IsNullOrEmpty(value)) Temperature = value.Substring(null, ":").ToDouble();
        }
        ...
        if (!_excludes.Contains(nameof(CpuRate)) && File.Exists(file))
        {
            ...
            try
            {
                ...
                {
                    var vs = line.TrimStart("cpu").Trim().ToString().Split(' ');
                    var current = new SystemTime
                    {
                        IdleTime = vs[3].ToLong(),
                        TotalTime = vs.Take(7).Sum(e => e.ToLong()).ToLong(),
                    };
    ...
    {
        const string fr = "/etc/redhat-release";
        if (TryRead(fr, out var value)) return value;

        const string dr = "/etc/debian-release";
        if (TryRead(dr, out value)) return value;

        const string sr = "/etc/os-release";
        if (TryRead(sr, out value)) return value?.SplitAsDictionary("=", "\n", true)["PRETTY_NAME"].Trim();

        var uname = ProcessHelper.Execute("uname", "-sr", 0, false)?.Trim();
        if (!String.IsNullOrEmpty(uname))
        {
            ...
            {
                if (!string.IsNullOrEmpty(item) && item.StartsWithIgnoreCase("Android")) return item;
            }
    ...
    {
        var str = ProcessHelper.Execute(cmd, arguments, 0, false);
    ...
    public static IDictionary<String, String> ReadPowerShell(String command)
    {
        ...
        var str = ProcessHelper.Execute("powershell.exe", args, 3_000) ?? String.Empty;
    ...
    public static IDictionary<String, String> ReadWmic(String type, params String[] keys)
    {
        ...
        var str = ProcessHelper.Execute("wmic", args, 0, false)?.Trim();
        if (string.IsNullOrEmpty(str)) return dic2;
        ...
        {
            dic2[item.Key] = item.Value.OrderBy(static e => e).Join();
        }
    ...
    public static IDictionary<String, String?> ReadDeviceInfo()
    {
        ...
        {
            var type = Type.GetType("Android.OS.Build");
            if (type != null)
            {
        ...
        {
            var type = Type.GetType("Xamarin.Essentials.DeviceInfo");
            if (type != null)
            {
                foreach (var item in type.GetProperties(BindingFlags.Public | BindingFlags.Static))
                {
        ...
        {
            var type = Type.GetType("Android.Provider.Settings")?.GetNestedType("Secure");
            if (type != null)
            {
                var data = Type.GetType("Android.App.Application");
                var resolver = data?.GetValueEx(data, "Context")?.GetValueEx(data, "ContentResolver");
                if (resolver != null)
                {
                    const string name = "android_id";
                    dic[name] = type.InvokeEx(type, "GetString", resolver, name) as String;
                }
            }
        }
    ...
    public static IDictionary<String, Object?> ReadDeviceBattery()
    {
        ...
        var type = Type.GetType("Xamarin.Essentials.Battery");
    ...
    [return: MarshalAs(UnmanagedType.Bool)]
    [SecurityCritical]
    internal static partial Boolean GlobalMemoryStatusEx(ref MEMORYSTATUSEX lpBuffer);
#endif
    ...
    [return: MarshalAs(UnmanagedType.Bool)]
    [SecurityCritical]
    private static partial Boolean GetSystemTimes(out FILETIME idleTime, out FILETIME kernelTime, out FILETIME userTime);
#endif
    ...
    public static String GetInfo(String path, String property, String? nameSpace = null)
    {
        ...
        try
        {
            ...
            foreach (var mo in moc)
            {
                ...
                if (val != null)
                {
                    var v = val.ToString()?.Trim().TrimInvisible();
                    if (v != null) bbs.Add(v);
                }
            }
        }
