...
public sealed class WaitLock : IDisposable
{
    ...
    public WaitLock(string name = nameof(WaitLock), int maxCount = 1, bool initialZeroState = false)
    {
        _name = name;
        if (initialZeroState)
    ...
    private static Task<bool> WaitUntilCountOrTimeoutAsync(Task<bool> asyncWaiter, int millisecondsTimeout, CancellationToken cancellationToken)
    {
