...
namespace ThingsGateway.Foundation.Common.PooledAwait
{
    ...
    /// </summary>
    public readonly struct LazyTaskCompletionSource<T> : IDisposable, IEquatable<LazyTaskCompletionSource<T>>
    {
        ...
        public bool Equals(LazyTaskCompletionSource<T> ltcs) => _state == ltcs._state && _token == ltcs._token;
        ...
        /// </summary>
        public Task<T> Task => (Task<T>)(_state?.GetTask(_token) ?? PooledAwaitThrowHelper.ThrowInvalidOperationException<Task<T>>());
        ...
        {
            if (!TrySetResult(result)) PooledAwaitThrowHelper.ThrowInvalidOperationException();
        }
        ...
        {
            if (!TrySetCanceled(cancellationToken)) PooledAwaitThrowHelper.ThrowInvalidOperationException();
        }
        ...
        {
            if (!TrySetException(exception)) PooledAwaitThrowHelper.ThrowInvalidOperationException();
        }
        ...
        public static bool operator ==(LazyTaskCompletionSource<T> left, LazyTaskCompletionSource<T> right)
        {
