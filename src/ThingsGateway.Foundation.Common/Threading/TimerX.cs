...
public class TimerX : ITimer, ITimerx
{
    ...
    /// <summary>可重入</summary>
    public Boolean Reentrant { get; set; }
    ...
    private TimerX(Object? state, String? scheduler = null)
    {
        ...
        Scheduler = (string.IsNullOrEmpty(scheduler) != false) ? TimerScheduler.Default : TimerScheduler.Create(scheduler!);
        //Scheduler.Add(this);
    ...
    public Boolean Change(long dueTime, int period)
    {
        return Change((int)dueTime, period);
    }
    /// <summary>更改计时器的启动时间和方法调用之间的时间间隔。</summary>
    /// <param name="dueTime">一个 TimeSpan，表示在调用构造 ITimer 时指定的回调方法之前的延迟时间量。 指定 InfiniteTimeSpan 可防止重新启动计时器。 指定 Zero 可立即重新启动计时器。</param>
    /// <param name="period">构造 Timer 时指定的回调方法调用之间的时间间隔。 指定 InfiniteTimeSpan 可以禁用定期终止。</param>
