...
public class RedisStream<T> : QueueBase, IProducerConsumer<T>, IDisposable
{
    ...
    {
        ArgumentNullExceptionEx.ThrowIfNull(values);
    ...
    private Int32 RetryAck()
    {
        ...
        if (_nextRetry < now)
        {
            ...
            while (times-- > 0)
            {
                ...
                var p = id.IndexOf('-');
                if (p > 0) id = $"{id.AsSpan()[..p].ToLong() + 1}-0";
            }
    ...
    public IList<Message> Read(String startId, Int32 count)
    {
        ...
        if (rs?.Length == 1 && rs[0] is Object[] vs && vs.Length == 2)
        {
            /*
...
2)  1)  1)     "1593751768717-0"
    ...
    3)  1)     "1593751768733-0"
        2)  1)      "name"
            ...
            4)      "36"
             */
            if (vs[1] is Object[] vs2) return RedisStream<T>.Parse(vs2);
        }
    ...
    private static List<Message> Parse(Object[] vs)
    {
    ...
    public async Task ConsumeAsync(Func<T, Message, CancellationToken, Task> onMessage, CancellationToken cancellationToken = default)
    {
        ...
        while (!cancellationToken.IsCancellationRequested)
        {
            try
            {
                ...
                if (mqMsg?.Id.IsNullOrEmpty() == false)
                {
                    // 串联上下游调用链
