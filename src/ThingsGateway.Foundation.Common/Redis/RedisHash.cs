...
public class RedisHash<TKey, TValue> : RedisBase, IDictionary<TKey, TValue> where TKey : notnull
{
    #region 构造
    /// <summary>实例化</summary>
    ...
    public virtual IEnumerable<KeyValuePair<TKey, TValue>> Search(SearchModel model)
    {
        ...
        while (count > 0)
        {
            ...
            for (var i = 0; i < ps.Length - 1; i += 2)
            {
                if (count-- > 0)
                {
                    var key = (ps[i] as IPacket)!.ToStr().ChangeTypeEx<TKey>();
                    var val = (ps[i + 1] as IPacket)!.ToStr().ChangeTypeEx<TValue>();
