...
#pragma warning disable CA1835 // 对于 "ReadAsync" 和 "WriteAsync"，首选基于内存的重载
        var n = await ns.ReadAsync(pk.Buffer, pk.Offset, pk.Length, cancellationToken).ConfigureAwait(false);
#pragma warning restore CA1835 // 对于 "ReadAsync" 和 "WriteAsync"，首选基于内存的重载
        if (n <= 0) return [];
    ...
    private List<Object?> ParseResponse(Stream ms, Int32 count, ref BufferedReader reader)
    {
        /*
         * 响应格式
        ...
        for (var i = 0; i < count; i++)
        {
            ...
            {
                list.Add(RedisClient.ReadBlocks(ref reader, log));
            }
            else
            {
                // 字符串以换行为结束符
    ...
    private static Object?[] ReadBlocks(ref BufferedReader reader, StringBuilder? log)
    {
        // 结果集数量
        ...
        for (var i = 0; i < len; i++)
        {
            ...
            {
                arr[i] = RedisClient.ReadBlocks(ref reader, log);
            }
            else
            {
    ...
    /// <param name="cancellationToken">取消通知</param>
    /// <returns></returns>
    public virtual Task<Object?> ExecuteAsync(String cmd, Object?[] args, CancellationToken cancellationToken = default)
    {
        // 埋点名称，支持二级命令
        var act = cmd.EqualIgnoreCase("cluster", "xinfo", "xgroup", "xreadgroup") ? $"{cmd}-{args?.FirstOrDefault()}" : cmd;

        return ExecuteCommandAsync(cmd, args, cancellationToken);
    ...
    public virtual Boolean TryChangeType(Object value, Type type, out Object? target)
    {
        ...
        if (value is Object[] objs)
        {
            ...
            for (var i = 0; i < objs.Length; i++)
            {
                ...
                else if (objs[i]?.GetType().IsFrom(elmType) == true)
                    arr.SetValue(objs[i], i);
            }
            target = arr;
            return true;
