...
using ThingsGateway.Foundation.Common.Caching.Clusters;
using ThingsGateway.Foundation.Common.Caching.Models;
using ThingsGateway.Foundation.Common.Caching.Queues;
using ThingsGateway.Foundation.Common.Caching.Services;
using ThingsGateway.Foundation.Common.Collections;
using ThingsGateway.Foundation.Common.Data;
using ThingsGateway.Foundation.Common.Messaging;
...
public class FullRedis : Redis
{
    ...
    public FullRedis(IServiceProvider provider, RedisOptions options) : this(options) { }
    ...
    public override void Init(String config)
    {
        ...
        var dic = Redis.ParseConfig(config);
        if (dic.Count > 0)
        {
...
#pragma warning restore CA1849 // 当在异步方法中时，调用异步方法
    ...
    {
        if (keys?.Any() != true) return new Dictionary<String, T>();
    ...
    {
        GetHashMap<T>(key).Add(hashKey, value);
    }
    ...
    {
        return GetHashMap<T>(key).HMGet(field)[0];
    }

    public override IDictionary<string, T> HashGetAll<T>(string key)
    {
        return GetHashMap<T>(key).GetAll();
    }
    ...
    public override long DelByPattern(string pattern)
    {
        ...
        var enumerable = Keys?.Where((string k) => k.StartsWith(pattern2));
        if (enumerable?.Any() == true)
        {
            return Remove(enumerable.ToArray());
        }
