...
public abstract class Redis : Cache, IConfigMapping, ILogFeature
{
    #region 属性
    ...
    public override void Init(String config)
    {
        ...
        if (pk?.Secret != null) connStr = pk.Unprotect(connStr);

        // 默认分号分割，旧版逗号分隔。可能只有一个server=后续多个含逗号的地址
        var dic = Redis.ParseConfig(connStr);
        if (dic.Count > 0)
        {
    ...
    /// <returns></returns>
    protected static IDictionary<String, String> ParseConfig(String connStr)
    {
        // 默认分号分割，旧版逗号分隔。可能只有一个server=后续多个含逗号的地址
        var dic =
    ...
    {
        if (section?.Value != null) Init(section.Value);
    }
    #endregion
    ...
    /// <param name="ex"></param>
    /// <returns></returns>
    internal static Boolean NoDelay(Exception ex) => ex is SocketException or IOException or InvalidOperationException;
