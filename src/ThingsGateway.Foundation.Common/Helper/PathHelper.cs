...
using ThingsGateway.Foundation.Common.Compression;
...
public static class PathHelper
{
    ...
    public static void Compress(this FileInfo fi, String destFile)
    {
        ...
        else if (destFile.EndsWithIgnoreCase(".tar", ".tar.gz", ".tgz"))
        {
            ...
            if (destFile.EndsWithIgnoreCase(".tar"))
            {
                ...
                using var tarWriter = new System.Formats.Tar.TarWriter(fs, System.Formats.Tar.TarEntryFormat.Pax, false);
            ...
            else
            {
                ...
                using var tarWriter = new System.Formats.Tar.TarWriter(gs, System.Formats.Tar.TarEntryFormat.Pax, false);
                tarWriter.WriteEntry(fi.FullName, fi.Name);
    ...
    public static String[] CopyIfNewer(this DirectoryInfo di, String[] source, String? exts = null, Boolean allSub = false)
    {
        ...
        {
            // 跳过当前目录
            if (Path.Combine(AppContext.BaseDirectory, item).EqualIgnoreCase(cur)) continue;
    ...
    public static string GetRelativePath(string relativeTo, string path)
    {
        ...
        if (!isUnc)
        {
            if (!string.Equals(Path.GetPathRoot(relativeTo), Path.GetPathRoot(path), StringComparison.OrdinalIgnoreCase))
                return path; // 不同驱动器，返回绝对路径
        }

        var relativeUri = new Uri(relativeTo + Path.DirectorySeparatorChar);
        var pathUri = new Uri(path);

        var relUri = relativeUri.MakeRelativeUri(pathUri);
    ...
    /// <param name="ps"></param>
    /// <returns></returns>
    public static string CombinePathReplace(params string[] ps)
    {
        if (ps == null || ps.Length == 0)
            return string.Empty;

        var path = Path.Combine(ps);
        path = path.Replace('\\', '/');
        return path;
    }
    ...
    /// <returns></returns>
    public static FileInfo AsFile(this String file) => new(file);

    /// <summary>路径作为目录信息</summary>
    /// <param name="dir"></param>
    /// <returns></returns>
    public static DirectoryInfo AsDirectory(this String dir) => new(dir);
    ...
    public static String EnsureDirectory(this String path, Boolean isfile = true)
    {
        ...
        var dir = path;
        // 斜杠结尾的路径一定是目录，无视第二参数
        if (isfile || dir[^1] == Path.DirectorySeparatorChar)
