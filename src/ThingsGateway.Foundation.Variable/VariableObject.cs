...
public abstract class VariableObject
{
    ...
    /// <returns></returns>
    public virtual JsonNode GetExpressionsValue(object value, VariableRuntimeProperty variableRuntimeProperty)
    {
        var jToken = JsonUtil.GetJsonNodeFromObj(value);
        if (!string.IsNullOrEmpty(variableRuntimeProperty.Attribute.WriteExpressions))
        {
            ...
            jToken = JsonUtil.GetJsonNodeFromObj(data);
        }
    ...
    public virtual ReadOnlyMemory<byte> GetBytes(Expression<Func<object>> accessor)
    {
        ...
        return variable.VariableClass.BitConverter.GetBytesFromData(data, variable.VariableClass.DataType);
    }
    ...
    public virtual void SetValue()
    {
        ...
        foreach (var pair in VariableRuntimePropertyDict)
        {
            if (!string.IsNullOrEmpty(pair.Value.Attribute.ReadExpressions))
            {
                ...
                pair.Value.Property.SetValue(this, data.ChangeTypeEx(pair.Value.Property.PropertyType));
            }
            else
            {
                pair.Value.Property.SetValue(this, pair.Value.VariableClass.Value.ChangeTypeEx(pair.Value.Property.PropertyType));
            }
        }
    ...
    public virtual ValueTask<OperResult> WriteValueAsync(string propertyName, object value, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var jToken = GetExpressionsValue(value, variableRuntimeProperty);

            return Device.WriteAsync(variableRuntimeProperty.VariableClass.RegisterAddress, jToken, variableRuntimeProperty.VariableClass.DataType, cancellationToken: cancellationToken);
        }
        catch (Exception ex)
        {
            return EasyValueTask.FromResult(new OperResult(ex));
    ...
    protected virtual void GetVariableSources()
    {
        if (DeviceVariableSourceReads == null)
        {
            ...
            DeviceVariableSourceReads = Device.LoadSourceRead<VariableSourceClass, VariableClass>(variableClasss, MaxPack, "1000");
        }
    }
}
