...
internal static class PackHelper
{
    ...
    public static List<T> LoadSourceRead<T, T2>(SiemensS7Master device, IEnumerable<T2> deviceVariables, int maxPack, string defaultIntervalTime) where T : IVariableSource<T2>, new() where T2 : IVariable

    {
        ...
        {
            // 使用字典存储地址和对应的变量
            Dictionary<SiemensS7Address, T2> map = item.ToDictionary(it =>
            {
                // 解析SiemensS7Address对象
                var s7Address = SiemensS7Address.ParseFrom(it.RegisterAddress);
                // 根据地址获取转换参数
                if (it.BitConverter is S7BitConverter s7BitConverter)
                {
                ...
                if (lastLen <= 0)
                {
                    switch (it.DataType)
                    {
                        case DataTypeEnum.String:
                            if (it.BitConverter.StringLength == null)
                            {
                            ...
                            else
                            {
                                ...
                                {
                                    if (it.BitConverter.IsVariableStringLength)
                                    {
                                        // 字符串在S200Smart中，第一个字节不属于实际内容
                                        it.Index += 1;
                                        lastLen = it.BitConverter.StringLength.Value + 1;
                                    }
                                    else
                                    {
                                        lastLen = it.BitConverter.StringLength.Value;
                                    }
                                ...
                                else
                                {
                                    if (s7Address.WStringEnable)
                                    {
                                        ...
                                        it.Index += 4;
                                        lastLen = it.BitConverter.StringLength.Value + 4;
                                        //it.ThingsGatewayBitConverter.Encoding = Encoding.BigEndianUnicode;

                                    }
                                    else if (it.BitConverter.IsVariableStringLength)
                                    {
                                        // 字符串在S7中，前两个字节不属于实际内容
                                        it.Index += 2;
                                        lastLen = it.BitConverter.StringLength.Value + 2;
                                    }
                                    else
                                    {
                                        lastLen = it.BitConverter.StringLength.Value;
                ...
                // 对于计数器和定时器，长度需调整
                if ((s7Address.DataCode == S7Area.TM || s7Address.DataCode == S7Area.CT) && lastLen == 1)
                {
                    lastLen = 2;
                }
                if (it.ArrayLength != null)
                {
                    if (it.DataType == DataTypeEnum.Boolean)
                    {
                        var len = Math.Ceiling((decimal)it.ArrayLength.Value / 8);
                        lastLen *= (int)len;
                    }
                    else
                    {
                        lastLen *= it.ArrayLength.Value;
                    }
                }

                // 将变量的应读取长度写入SiemensS7Address实例中
            ...
            foreach (var functionCode in functionCodes)
            {
                ...
                foreach (var stationNumber in dbNumbers)
                {
                    ...
                    var tempResult = LoadSourceRead<T, T2>(addressList!, (byte)functionCode, item.Key, device);
    ...
    /// </summary>
    /// <param name="addressList">SiemensS7Address和IVariable的字典，表示地址和对应的变量</param>
    ...
    private static List<T> LoadSourceRead<T, T2>(Dictionary<SiemensS7Address, T2> addressList, byte functionCode, string intervalTime, SiemensS7Master siemensS7Net) where T : IVariableSource<T2>, new() where T2 : IVariable
    {
        ...
        {
            int address = 0;
            if (it.DataCode == S7Area.TM || it.DataCode == S7Area.CT)
            {
        ...
        while (maxAddress >= minAddress) // 循环，直到最大地址小于最小地址
        {
            ...
            List<SiemensS7Address> tempAddresses = new(); // 临时地址列表用于存储分配给单个数据包的地址
            ...
            T sourceRead = new() // 创建一个新的源读取对象
            {
                ...
                DeviceAddress = new SiemensS7Address(tempAddresses.OrderBy(it => it.AddressStart).First()) { Length = sourceLen },
