...
public partial class SiemensS7Master : DeviceBase
{
    ...
    public override ValueTask<OperResult<ReadOnlyMemory<byte>>> ReadAsync(IDeviceAddress address, DataTypeEnum dataType, IThingsGatewayBitConverter bitConverter, CancellationToken cancellationToken = default)
    {
        try
        {
            if (address is SiemensS7Address sAddress)
            {
                return S7ReadAsync([sAddress], cancellationToken);
            }
            else
            {
                return EasyValueTask.FromResult(new OperResult<ReadOnlyMemory<byte>>(new ArgumentException("address must be of type SiemensS7Address", nameof(address))));
            }
        }
        catch (Exception ex)
        {
    ...
    public override async ValueTask<OperResult> WriteAsync(string address, ReadOnlyMemory<byte> value, DataTypeEnum dataType, IThingsGatewayBitConverter? bitConverter = null, CancellationToken cancellationToken = default)
    {
    ...
    protected override async ValueTask<bool> ChannelStarted(IClientChannel channel, bool last)
    {
        try
        {
            ...
            try
            {
                ...
                PduLength = BitConverter.ToUInt16(result2.Content.Span, 0) - 28;
    ...
    {
        return this.WriteAsync(address, Convert.ToUInt16((dateTime - S7DateTime.SpecMinimumDateTime).TotalDays), cancellationToken: cancellationToken);
    }
    ...
    {
        return WriteAsync(address, S7DateTime.ToByteArray(dateTime), DataTypeEnum.Byte, cancellationToken: cancellationToken);
    }
    ...
    public override async ValueTask<OperResult<string[]>> ReadStringAsync(string address, int length, IThingsGatewayBitConverter bitConverter = null, CancellationToken cancellationToken = default)
    {
        ...
        if (((S7BitConverter)bitConverter)?.WStringEnable == true)
        {
            ...
            var result = await SiemensHelper.ReadWStringAsync(this, address, bitConverter, bitConverter.Encoding, cancellationToken).ConfigureAwait(false);
            if (result.IsSuccess)
            {
        ...
        else if (bitConverter.IsVariableStringLength)
        {
            ...
            var result = await SiemensHelper.ReadStringAsync(this, address, bitConverter, bitConverter.EncodingValue, cancellationToken).ConfigureAwait(false);
            if (result.IsSuccess)
            {
                return OperResult.CreateSuccessResult(new string[] { result.Content });
            }
    ...
    public override ValueTask<OperResult> WriteAsync(string address, string value, IThingsGatewayBitConverter bitConverter = null, CancellationToken cancellationToken = default)
    {
        ...
        if (((S7BitConverter)bitConverter)?.WStringEnable == true)
        {
            return SiemensHelper.WriteWStringAsync(this, address, value, bitConverter, bitConverter.Encoding, cancellationToken);
        }
        if (bitConverter.IsVariableStringLength)
        {
            return SiemensHelper.WriteStringAsync(this, address, value, bitConverter, bitConverter.EncodingValue, cancellationToken);
        }
        else
        {
