...
namespace Westwind.Scripting
{
    ...
    public class ScriptParser
    {
            ...
            string basePath = null)
        {
            ...
            string result = await ScriptEngine.ExecuteCodeAsync(code, model).ConfigureAwait(false) as string;
            ...
            string basePath = null)
        {
            ...
            string result = await ScriptEngine.ExecuteCodeAsync<string, TModelType>(code, model).ConfigureAwait(false) as string;
        ...
        /// along with the error message and the generated code.
        /// </summary>
        /// <param name="scriptFile">The template to execute that contains C# script</param>
        /// <param name="model">A model that can be accessed in the template as `Model`. Pass null if you don't need to access values.</param>
        /// <param name="scriptEngine">Optional CSharpScriptEngine so you can customize configuration and capture result errors</param>
        /// <param name="basePath">Optional basePath/root for the script and related partials so ~/ or / can be resolved</param>
        /// <returns>expanded template or null. On null check `scriptEngine.Error` and `scriptEngine.ErrorMessage`</returns>
            ...
            string basePath = null)
        {
            ...
            return await ScriptEngine.ExecuteCodeAsync(code, model).ConfigureAwait(false) as string;
        }
            ...
            string basePath = null)
        {
            ...
            return await ScriptEngine.ExecuteCodeAsync<string, TModelType>(code, model).ConfigureAwait(false) as string;
        }
        ...
        protected void ParseSections(ScriptFileContext scriptContext)
        {
            ...
            foreach (Match match in matches)
            {
                ...
                string sectionContent = StringUtils.ExtractString(section, match.Groups[1].Value, match.Groups[3].Value)?.TrimStart(trimChars);
        ...
        public string ParseScriptToCode(ScriptFileContext scriptContext)
        {
            ...
            using var literal = new ValueStringBuilder();
...
";
                ...
                while (atStart > -1)
                {
                    ...
                    literal.Append(scriptText.AsSpan(0, atStart));
