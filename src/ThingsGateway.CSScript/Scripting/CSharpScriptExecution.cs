    ...
    public class CSharpScriptExecution
    {
        ...
        public object ExecuteMethod(string code, string methodName, params object[] parameters)
        {
            ...
            else
            {
                ...
                if (!CachedAssemblies.TryGetValue(hash, out Assembly? value))
                {
...
#pragma warning restore CA1849 // 当在异步方法中时，调用异步方法
        ...
        /// </summary>
        /// <param name="code">The code to execute</param>
        /// <param name="model">The parameters to pass the code
        /// You can reference parameters as @0, @1, @2 in code to map
        /// to the parameter array items (ie. @1 instead of parameters[1])
        /// </param>
        /// <returns>Result cast to a type you specify</returns>
        ...
        public static bool MarkDelete(string path)
        {
        ...
        public object CompileClassWithFile(string code)
        {
            var type = CompileClassToTypeWithFile(code);
        ...
        /// <param name="code">Fully self-contained C# class</param>
        /// <returns>Instance of that class or null</returns>
        public Type CompileClassToTypeWithFile(string code)
        {
            ...
            {
                var hash = Fnv1a64(code).ToString();
                var dir = Path.Combine(AppContext.BaseDirectory, "CSSCRIPT");
                Directory.CreateDirectory(dir);
                OutputAssembly = Path.Combine(dir, $"{hash}.dll");
            }
            if (DisableAssemblyCaching)
            {
                if (!CompileAssemblyWithFile(code))
                    throw LastException;
        ...
        /// </summary>
        private bool CompileAssemblyWithFile(string code)
        {
            Assembly = null;

            Assembly = GetAssembly(OutputAssembly);

            if (Assembly == null)
            {
                AddLoadedReferences();
                if (!CompileAssembly(code, true))
                {
                    return false;
                }
                else
        ...
        /// <summary>
        /// <para>Parses references with this syntax:</para>
        /// <para>#r assembly.dll</para>
        /// <para>Each match found is added to the assembly list</para>
        /// </summary>
        ...
        public static string[] GetLines(string s, int maxLines = 0)
        {
            ...
            return s.Split(separator, maxLines);
        }
        ...
        private Assembly LoadAssemblyFrom(Stream stream)
        {
#if NET6_0_OR_GREATER
            if (AlternateAssemblyLoadContext != null)
            {
                return AlternateAssemblyLoadContext.LoadFromStream(stream);
            }
#endif
            return Assembly.Load(stream.ReadBytes(-1));
        }
        #endregion
