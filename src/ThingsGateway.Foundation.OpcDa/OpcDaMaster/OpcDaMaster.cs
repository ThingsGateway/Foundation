...
public class OpcDaMaster : IDisposable
{
    ...
    public Dictionary<string, List<OpcItem>> AddItemsWithSave(List<string> items)
    {
        lock (lockThis)
        {
            ...
            var addItems = items.ConvertAll(o => new OpcItem(o)).ChunkBetter(OpcDaProperty.GroupSize).ToDictionary(a => "default" + (i++));
    ...
    /// </summary>
    /// <returns></returns>
    public Dictionary<string, (bool, string)> WriteItem(Dictionary<string, object> writeInfos)
    {
        if (IsExit == 1) throw new ObjectDisposedException(nameof(OpcDaMaster));
        Dictionary<string, (bool, string)> results = new();
        ...
        foreach (var item1 in valueGroup)
        {
            try
            {
                if (item1.Key == null)
                {
                    ...
                    {
                        results.AddOrUpdate(item2.Key, ValueTuple.Create(false, $"The variable does not exist {item2.Key}"));
                    }
                }
                else
                {
                    ...
                    {
                        results.AddOrUpdate(handleItems[item2.Item1].ItemID, ValueTuple.Create(false, $"Error code{item2.Item2}"));
                    }
                }
                foreach (var item2 in item1)
                {
                    results.AddOrUpdate(item2.Key, ValueTuple.Create(true, $"Success"));
                }
            }
            catch (Exception ex)
            {
                ...
                {
                    results.AddOrUpdate(item, ValueTuple.Create(false, ex.Message));
                }
                return results;
            }
        }
