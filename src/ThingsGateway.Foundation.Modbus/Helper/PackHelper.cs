...
public static class PackHelper
{
    ...
    /// <returns></returns>
    /// <exception cref="ArgumentNullException"><paramref name="deviceVariables"/> is <c>null</c>.</exception>
    public static List<T> LoadSourceRead<T, T2>(IModbusAddress device, IEnumerable<T2> deviceVariables, int maxPack, string defaultIntervalTime, byte? station = null) where T : IVariableSource<T2>, new() where T2 : IVariable
    {
        // 检查参数是否为空
        ArgumentNullExceptionEx.ThrowIfNull(deviceVariables, nameof(deviceVariables));
        ...
        foreach (var group in deviceVariableRuntimeGroups)
        {
            ...
            Dictionary<ModbusAddress, T2> map = group.ToDictionary(it =>
            {
                ...
                if (lastLen <= 0)
                {
                    ...
                    switch (it.DataType)
                    {
                        case DataTypeEnum.String:
                            // 字符串类型需特殊处理
                            lastLen = (it.BitConverter?.StringLength == null ? throw new(ThingsGateway.Foundation.AppResource.StringTypePackError) : it.BitConverter.StringLength.Value);
                            break;
                ...
                var result = device.GetModbusAddress(address, station);
                ArgumentNullExceptionEx.ThrowIfNull(result, nameof(ModbusAddress));
                // 如果是数组，需要乘以数组长度
            ...
            // 获取所有功能码
            foreach (var functionCode in modbusAddressList.Select(t => t.FunctionCode).Distinct())
            {
                // 获取相同功能码的地址
                var modbusAddressSameFunList = modbusAddressList.Where(t => t.FunctionCode == functionCode);
                // 获取相同站号的地址
                foreach (var stationNumber in modbusAddressSameFunList.Select(t => t.Station).Distinct())
                {
                    // 获取相同站号的地址
    ...
    private static List<T> LoadSourceRead<T, T2>(Dictionary<ModbusAddress, T2> addressList, int functionCode, string intervalTime, int maxPack) where T : IVariableSource<T2>, new() where T2 : IVariable

    {
        ...
        while (maxAddress >= minAddress)
        {
            ...
            T sourceRead = new()
            {
                ...
                DeviceAddress = new ModbusAddress(startAddress) { Length = (ushort)sourceLen },
