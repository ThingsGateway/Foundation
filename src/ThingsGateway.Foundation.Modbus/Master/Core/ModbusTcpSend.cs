...
public class ModbusTcpSend : ISendMessage
{
    ...
    public void Build<TByteBlock>(ref TByteBlock byteBlock) where TByteBlock : IBytesWriter
    {
        ...
        if (!Read)
        {
            ...
            {
                ModbusAddress.WriteFunctionCode = ModbusAddress.WriteFunctionCode ?? 0 + 0x30;
            }
        }

        WriterExtension.WriteValue(ref byteBlock, (ushort)TransactionId, EndianType.Big);
        WriterExtension.WriteValue(ref byteBlock, (ushort)DeviceId, EndianType.Big);

        if (Read)
        {
            WriterExtension.WriteValue(ref byteBlock, (ushort)6, EndianType.Big);
        ...
        else if (wf == 15 || wf == 16)
        {
            ...
            WriterExtension.WriteValue(ref byteBlock, (byte)ModbusAddress.WriteFunctionCode);
            WriterExtension.WriteValue(ref byteBlock, (ushort)ModbusAddress.StartAddress, EndianType.Big);
            var len = (ushort)Math.Ceiling(wf == 15 ? ModbusAddress.Length : ModbusAddress.Length / 2.0);
            WriterExtension.WriteValue(ref byteBlock, len, EndianType.Big);
            WriterExtension.WriteValue(ref byteBlock, (byte)(data.Length));
