...
public class ModbusSlave : DeviceBase, IModbusAddress
{
    ...
    private Task<bool> HandleWriteCoreAsync(ModbusAddress address, IClientChannel client, ModbusRequest modbusRequest)
    {
        ...
        static async PooledTask<bool> HandleWriteCoreAsync(ModbusSlave @this, ModbusAddress address, IClientChannel client, ModbusRequest modbusRequest)
        {
            ...
            {
                var result = await @this.WriteData(address, @this.BitConverter, client).ConfigureAwait(false);
        ...
        bool modbusRtu)
    {
        if (modbusRtu)
            ...
            ByteBlockHelper.Write(ref byteBlock, sequences.Slice(0, 8));

        if (modbusRequest.IsBitFunction)
        {
            var bitdata = content.Span.ByteToByteArray();
    ...
    private Task WriteError(bool modbusRtu, IClientChannel client, ReadOnlySequence<byte> bytes, ReceivedDataEventArgs e)
    {
        ...
        static async PooledTask WriteError(ModbusSlave @this, bool modbusRtu, IClientChannel client, ReadOnlySequence<byte> bytes, ReceivedDataEventArgs e)
        {
            ...
            try
            {
                if (modbusRtu)
                {
                    ByteBlockHelper.Write(ref byteBlock, bytes.Slice(0, 2));
                    WriterExtension.WriteValue(ref byteBlock, (byte)1);
                    byteBlock.Write(CrcHelper.Crc16Only(byteBlock.Span));
                    ByteBlockHelper.WriteBackValue(ref byteBlock, (byte)(byteBlock.Span[1] + 128), EndianType.Big, 1);
                }
                else
                {
    ...
    private Task WriteSuccess(bool modbusRtu, IClientChannel client, ReadOnlySequence<byte> bytes, ReceivedDataEventArgs e)
    {
        ...
        static async PooledTask WriteSuccess(ModbusSlave @this, bool modbusRtu, IClientChannel client, ReadOnlySequence<byte> bytes, ReceivedDataEventArgs e)
        {
            ...
            try
            {
                if (modbusRtu)
                {
                    ByteBlockHelper.Write(ref byteBlock, bytes.Slice(0, 6));
                    byteBlock.Write(CrcHelper.Crc16Only(byteBlock.Span));
                }
                else
                {
    ...
    public override ValueTask<OperResult<ReadOnlyMemory<byte>>> ReadAsync(IDeviceAddress address, DataTypeEnum dataType, IThingsGatewayBitConverter bitConverter, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            {
                return EasyValueTask.FromResult(ModbusRequest(mAddress, true, cancellationToken));
            }
            else
            {
    ...
    public override ValueTask<OperResult<ReadOnlyMemory<byte>>> ReadAsync(string address, int length, DataTypeEnum dataType, IThingsGatewayBitConverter? bitConverter = null, CancellationToken cancellationToken = default)
    {
        ...
        if (result.IsSuccess)
        {
            if (dataType == DataTypeEnum.Boolean && mAddress.IsBitFunction)
            {
                var bitdata = result.Content.Span.ByteToByteArray();
                return EasyValueTask.FromResult(new OperResult<ReadOnlyMemory<byte>>() { Content = bitdata });
            }
    ...
    private ValueTask<OperResult> WriteAsync(ReadOnlyMemory<byte> value, DataTypeEnum dataType, ModbusAddress? mAddress, CancellationToken cancellationToken)
    {
            ...
            try
            {
                ...
                else
                {
                    if (mAddress.BitIndex < 16)
                    {
                        ...
                        mAddress.SlaveWriteDatas = new(BitConverter.GetBytes(writeData));
                        ModbusRequest(mAddress, false, cancellationToken);
