...
public delegate void JTokenDataChangedEventHandler((VariableNode variableNode, MonitoredItem monitoredItem, DataValue dataValue, JToken jToken) value);
/// <summary>
/// 订阅委托
/// </summary>
/// <param name="value"></param>
public delegate void JsonNodeDataChangedEventHandler((VariableNode variableNode, MonitoredItem monitoredItem, DataValue dataValue, JsonNode jsonNode) value);
...
public class OpcUaMaster : IAsyncDisposable
{
    ...
    public async Task AddSubscriptionAsync(string subscriptionName, string[] items, bool loadType = true, CancellationToken cancellationToken = default)
    {
        ...
        if (isError)
        {
            Log(3, null, $"Failed to create subscription for the following variables：{Environment.NewLine}{JsonConvert.SerializeObject(
                m_subscription.MonitoredItems.Where(
                a => a.Status.Error != null && StatusCode.IsBad(a.Status.Error.StatusCode))
                .Select(a => $"{a.StartNodeId}：{a.Status.Error}"))}");
        }
    ...
    /// 从服务器读取值
    /// </summary>
    public Task<List<(string, DataValue, JToken)>> ReadJTokenValueAsync(string[] tags, CancellationToken cancellationToken = default)
    {
        return ReadJTokenValueAsync(tags.Select(a => new NodeId(a)).ToArray(), cancellationToken);
    }
    /// <summary>
    /// 从服务器读取值
    /// </summary>
    public Task<List<(string, DataValue, JsonNode)>> ReadJsonNodeValueAsync(string[] tags, CancellationToken cancellationToken = default)
    {
        return ReadJsonNodeValueAsync(tags.Select(a => new NodeId(a)).ToArray(), cancellationToken);
    }
    /// <summary>
    /// 读取一个节点的所有属性
    /// </summary>
    ...
    /// 异步写opc标签
    /// </summary>
    public async Task<Dictionary<string, ValueTuple<bool, string>>> WriteNodeAsync(Dictionary<string, JToken> writeInfoLists, CancellationToken cancellationToken = default)
    {
        Dictionary<string, ValueTuple<bool, string>> results = new();
        try
        {
     ...
     nodesToWrite: valuesToWrite, cancellationToken).ConfigureAwait(false);
            ...
            for (int i = 0; i < keys.Count; i++)
            {
                if (!StatusCode.IsGood(result.Results[i]))
                    results.Add(keys[i], ValueTuple.Create(false, result.Results[i].ToString()));
                else
                {
                    results.Add(keys[i], ValueTuple.Create(true, "Success"));
                }
            }
        ...
        catch (Exception ex)
        {
            ...
            {
                results.Add(item, ValueTuple.Create(false, ex.Message));
            }
            return results;
        }
    }
    ...
    /// 异步写opc标签
    /// </summary>
    public async Task<Dictionary<string, ValueTuple<bool, string>>> WriteNodeAsync(Dictionary<string, JsonNode> writeInfoLists, CancellationToken cancellationToken = default)
    {
        Dictionary<string, ValueTuple<bool, string>> results = new();
        try
        {
    ...
    /// 从服务器读取值
    /// </summary>
    private async Task<List<(string, DataValue, JsonNode)>> ReadJsonNodeValueAsync(NodeId[] nodeIds, CancellationToken cancellationToken = default)
    {
        if (m_session == null)
        {
            throw new ArgumentNullException(nameof(m_session));
