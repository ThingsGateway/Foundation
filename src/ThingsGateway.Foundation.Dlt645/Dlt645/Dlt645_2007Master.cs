...
public class Dlt645_2007Master : DtuDeviceBase
{
    ...
    {
        if (address is Dlt645_2007Address dlt645_2007Address)
        {
            return Dlt645RequestAsync(dlt645_2007Address, ControlCode.Read, FEHead, cancellationToken: cancellationToken);
        }
        else
        {
            return EasyValueTask.FromResult(new OperResult<ReadOnlyMemory<byte>>(new ArgumentException("address must be of type Dlt645_2007Address", nameof(address))));
        }
    }
    ...
    public override async ValueTask<OperResult> WriteAsync(string address, ReadOnlyMemory<byte> value, DataTypeEnum dataType, IThingsGatewayBitConverter? bitConverter = null, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var codes = ArrayHelper.SpliceArray(Password.HexStringToBytes(), OperCode.HexStringToBytes());
            var dAddress = Dlt645_2007Address.ParseFrom(address, Station);
    ...
    {
        return WriteAsync(address, new string[] { value }.AsMemory(), bitConverter, cancellationToken);
    }
    ...
    public ValueTask<OperResult<ReadOnlyMemory<byte>>> WritePasswordAsync(byte level, string oldPassword, string newPassword, string station = null, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var bytes = ArrayHelper.SpliceArray(levelstr
                , newPasswordstr
                , oldPasswordstr);
