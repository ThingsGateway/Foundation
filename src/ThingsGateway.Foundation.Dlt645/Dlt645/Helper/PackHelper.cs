...
internal static class PackHelper
{
    ...
    /// <returns></returns>
    public static List<T> LoadSourceRead<T, T2>(IDevice device, IEnumerable<T2> deviceVariables, int maxPack, string defaultIntervalTime, string? station = null) where T : IVariableSource<T2>, new() where T2 : IVariable
    {
        ...
        foreach (var item in group)
        {
            var r = new T()
            {
                ...
                DeviceAddress = Dlt645_2007Address.ParseFrom(item.Key, station),
                Length = 1,
